<template>
  <div class="task-page">
    ...
    <v-container>
        ...
        <v-col lg="2" md="2" class="d-flex align-center">
          ...
          <v-dialog v-model="filtersDialog" scrim="#9A8866">
            ...
            <v-row class="justify-center">
              <v-col cols="auto">
                <v-card
                  :title="$t('page.tasks.filters.title')"
                      ...
                      <v-row justify="end">
                        <v-col cols="12" md="6">
                          <v-btn
                            ...
                            >{{ $t("page.tasks.filters.close") }}</v-btn
                          >
                        </v-col>
                      </v-row>
                    </v-container>
      ...
      <v-row
        v-if="
          !loadingTasks && !reloadingTasks && (visibleTasks?.length || 0) == 0
        "
      >
        <v-col cols="12">
          ...
          <v-expansion-panels v-model="expandMap">
            <v-expansion-panel>
              ...
              <v-expansion-panel-text>
                <tarkov-map
                  v-if="selectedMergedMap"
                  :map="selectedMergedMap"
                  :marks="visibleGPS"
                />
                <v-alert v-else type="error"
                  >No map data available for this selection.</v-alert
                >
              </v-expansion-panel-text>
            </v-expansion-panel>
          </v-expansion-panels>
...
function timeUpdate() {
  ...
  const tarkovTime = new Date(timeAtTarkovSpeed + 3 * oneHour);
    ...
    for (const objective of task.objectives) {
      ...
      if (objective && objective.id && objectiveHasLocation(objective)) {
        ...
        if (activeUserView.value == "all") {
          ...
          const users = unlockedUsers.filter((user) => {
    ...
    if (activeUserView.value == "all") {
      ...
      if (activeSecondaryView.value == "available") {
        ...
        for (const task of visibleTaskList) {
          ...
          for (const teamId of Object.keys(
